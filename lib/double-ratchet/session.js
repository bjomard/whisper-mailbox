...
export class DoubleRatchetSession {
  constructor(rootKey, localKeyPair, remotePublicKey, isSender) {
    ...
    if (isSender) {
      // Sender: perform DH ratchet to get sending chain
      ...
      // Receiver: wait for first message to initialize receiving chain
  ...
  encrypt(plaintext, associatedData = new Uint8Array(0)) {
    ...
    if (!this.sendingChain) {
      // Store previous receiving chain length
    ...
    // Create header with current DH public key
    const header = {
  ...
  decrypt(ciphertext, mac, header, associatedData = new Uint8Array(0)) {
    ...
    if (needsRatchet) {
      // Skip messages in old receiving chain if exists
      if (this.receivingChain) {
        this.skipMessageKeys(header.previousChainLength);
      }
      
      // Perform DH ratchet to get new receiving chain
      const { chainKey } = this.dhRatchet.ratchetReceive(dhPublicKey);
      this.receivingChain = new SymmetricRatchet(chainKey);
      
      // Reset sending chain (will be recreated on next send)
      this.sendingChain = null;
    }
    
    // Skip messages if needed (for out-of-order delivery)
