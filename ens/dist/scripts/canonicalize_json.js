...
function mustKeyTypeFromSpkiB64u(spki_b64u, expectedType, field) {
...
function validateContactCard(card) {
    if (!card || typeof card !== "object")
        throw new Error("ContactCard: not an object");
    if (card.v !== 1)
        throw new Error(`ContactCard.v: expected 1, got ${card.v}`);
    if (card.service !== "whisper")
        throw new Error(`ContactCard.service: expected "whisper", got ${card.service}`);
    if (typeof card.handle !== "string" || !card.handle.trim())
        throw new Error("ContactCard.handle: missing");
    if (!card.pub || typeof card.pub !== "object")
        throw new Error("ContactCard.pub: missing");
    ...
    for (const [i, mb] of card.mailboxes.entries()) {
        if (!mb || typeof mb !== "object")
            throw new Error(`mailboxes[${i}]: not an object`);
        if (typeof mb.url !== "string" || !/^https?:\/\//.test(mb.url))
            throw new Error(`mailboxes[${i}].url: invalid`);
        if (typeof mb.id !== "string" || !mb.id.trim())
            throw new Error(`mailboxes[${i}].id: missing`);
        if (!Number.isInteger(mb.prio))
            throw new Error(`mailboxes[${i}].prio: must be integer`);
    }
