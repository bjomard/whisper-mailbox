...
type ContactCard = {
  ...
  service: string;
  handle: string;
  mailboxes?: Array<{ url: string; id: string; prio?: number }>;
  pub?: {
    // existing field in your cards:
    x25519_spki_b64u?: string;
    // optional raw pk (future-proof):
    x25519_pk_b64u?: string;
  };
};
 ...
 */
function x25519PkFromSpkiDer(der: Uint8Array): Uint8Array {
  ...
  for (let i = 0; i < der.length - 35; i++) {
    if (der[i] === 0x03) {
      ...
      if (len & 0x80) {
        const n = len & 0x7f;
        len = 0;
        for (let k = 0; k < n; k++) len = (len << 8) | der[off + k];
        off += n;
      }
...
async function readJsonMaybeUrl(p: string): Promise<any> {
  if (/^https?:\/\//i.test(p)) {
...
async function main() {
  ...
  const card = (await readJsonMaybeUrl(cardPathOrUrl)) as ContactCard;

  const pub = card.pub || {};
  let pkRaw: Uint8Array | null = null;
